...
%% Major code is from demo_fs_resource from webmachine
%% @author Trung Nguyen

-module(webui_resource).
-export([init/1]).
...
file_path(Context, Name) ->
	...
	FilePath = filename:join([Context#context.root, RelName]),
	%% error_logger:info_report([{context, Context}, {name, Name}, {relname, RelName}, {filepath, FilePath}]),
    FilePath.
...
accept_content(ReqData, Context) ->
	...
	error_logger:info_report([{filepath, FP}]),
